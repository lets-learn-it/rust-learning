...
fn print_help() {
  eprintln!("{} - replace a string with a new string", "FindNReplace".color("green"));
...
fn read_and_write(args: &Arguments) {
  ...
  let replace_data = replace(&args.pattern, &args.replace, &data).unwrap();
...
fn parse_args() -> Arguments {
  ...
  if args.len() != 4 {
      print_help();
      eprintln!("{}: wrong number of args. Expected 4 got {}", "Error".red().bold(), args.len());
      std::process::exit(1);
  }
